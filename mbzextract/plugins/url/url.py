...
class moodle_module:
    ...
    def parse(self):
        self.url_xml = et.parse(self.backup.open(
            self.directory + "/url.xml")).getroot()
        self.inforef_xml = et.parse(self.backup.open(
    ...
    def extract(self):
        ...
        if os.path.exists(os.path.join(self.final_dir, self.backup.stripped(self.url_xml.find('./url/name').text))) == False:
            os.makedirs(os.path.join(self.final_dir, self.backup.stripped(
                self.url_xml.find('./url/name').text)))
        os.chdir(os.path.join(self.final_dir, self.backup.stripped(
            self.url_xml.find('./url/name').text)))

        # write a .url file
        f = open(self.backup.stripped(
            self.url_xml.find('./url/name').text) + ".url", 'w')
        f.write("[InternetShortcut]\nURL=" +
                self.url_xml.find('.url/externalurl').text)
        f.close()

        # write a .desktop file for linux users
        f = open(self.backup.stripped(self.url_xml.find(
            './url/name').text) + ".desktop", 'w')
